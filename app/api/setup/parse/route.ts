...
import { ensureDirs, withStore, writeJson, writeProfileRaw } from "@shared/store";
...
import { hasGoogleCredentials, sheetUrlFromEnv, vercelEnvHelp } from "@shared/google-auth";
...
export async function POST(request: Request) {
  try {
    ...
    const sheetUrl = payload.google_sheet?.url?.trim() || sheetUrlFromEnv();
    if (isServerlessHost()) {
      if (!process.env.ANTHROPIC_API_KEY?.trim()) {
        return NextResponse.json({ error: vercelEnvHelp(["ANTHROPIC_API_KEY"]) }, { status: 400 });
      }
      if (!hasGoogleCredentials()) {
        return NextResponse.json({ error: vercelEnvHelp(["GOOGLE_SERVICE_ACCOUNT_JSON"]) }, { status: 400 });
      }
      if (!sheetUrl) {
        return NextResponse.json(
          { error: vercelEnvHelp(["GOOGLE_SHEET_URL"]) },
          { status: 400 },
        );
      }
      payload.google_sheet = { enabled: true, url: sheetUrl };
    } else if (payload.google_sheet?.enabled) {
